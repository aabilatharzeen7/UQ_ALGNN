...
def node_information_metrics(graph):
    ...
    for node in graph.nodes():
        # print (node)
        nodeInfo_dict[node] = {'degCent':degCent_measures[node], 'eigCent':eigCent_measures[node], 'betCent':betCent_measures[node], 'clsCent':clsCent_measures[node]}
    # Saving the metrics
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--queryStrategy', type=int, default = 2)
